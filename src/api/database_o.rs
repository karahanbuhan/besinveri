...
pub(crate) async fn search_food_by_tag_wild(
    ...
    for row in sqlx::query(
        "SELECT
            ...
            -- GROUP_CONCAT(DISTINCT T.description) AS all_tags,
            -- GROUP_CONCAT(DISTINCT A.description) AS all_allergens,
            -- GROUP_CONCAT(DISTINCT (SD.description || ':' || FSE.weight) AS all_servings

            FROM foods F

            -- Yemek etiketleri üzerine arama yapmak için hazırlık
            INNER JOIN food_tags FT ON F.id = FT.food_id
            INNER JOIN tags T ON FT.tag_id = T.id
            
            -- Yemeklerin resim adresini ve ekleyen kişinin ismine hazırlık
            LEFT JOIN food_images FI ON F.image_id = FI.id
            LEFT JOIN food_sources FS ON F.source_id = FS.id
            
            -- Yemeğin alerjenlerini almak için hazırlık
            -- LEFT JOIN food_allergens FA ON FA.food_id = F.id
            -- LEFT JOIN allergens A ON FA.allergen_id = A.id

            -- Bulunan yemeklerin tüm porsiyonları için
            -- LEFT JOIN food_servings FSE ON FSE.food_id = F.id
            -- LEFT JOIN servings_descriptions SD ON FSE.serving_description_id = SD.id

            WHERE T.description LIKE ?)
            
            -- Her satır bir yemeği temsil etsin istiyoruz
            -- GROUP BY F.id"*/,
    )
    .bind(format!("%{}%", tag))
    .fetch_all(pool)
